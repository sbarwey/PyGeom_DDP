...
class Multiscale_MessagePassing_UNet(torch.nn.Module):
    def __init__(self, 
        ...
        self.edge_aggregator = EdgeAggregation().jittable() # EdgeAggregation() object (defined below)
        self.act = act
    ...
    def forward(
        ...
        # ~~~~ Node Encoder: 
        for i,layer in enumerate(self.node_encode):
            x = layer(x) 
        ...
        # ~~~~ Edge Encoder: 
        for i,layer in enumerate(self.edge_encode):
        ...
        edge_mp_block = self.edge_down_mps[0] # get the edge mp block 
        node_mp_block = self.node_down_mps[0] # get the node mp block
        edge_norm_block = self.edge_down_norms[0] # layer norm for edge 
        node_norm_block = self.node_down_norms[0] # layer norm for node 
        n_mp = self.n_mp_down[0] # number of message passing blocks 
        
        for i, (edge_mlp, node_mlp, edge_norm, node_norm) in enumerate(zip(edge_mp_block,node_mp_block,edge_norm_block,node_norm_block)):
            # 1) concatenate edge + neighbor + owner into new edge_attr 
            x_own = x[edge_index[0,:], :]
            x_nei = x[edge_index[1,:], :]
            edge_attr_t = torch.cat((x_own, x_nei, edge_attr), dim=1)

            # 2) edge update mlp
            for j,layer in enumerate(edge_mlp): 
                edge_attr_t = layer(edge_attr_t) 
            ...
            x_t = torch.cat((x, edge_agg), dim=1)

            # 7) node update mlp
            for j,layer in enumerate(node_mlp):
                x_t = layer(x_t)
                if j < self.n_mlp_mp - 1:
                    x_t = self.act(x_t) 
        ...
        # ~~~~ Node Decoder: 
        for i,layer in enumerate(self.node_decode):
            x = layer(x)
            if i < self.n_mlp_encode - 1:
                x = self.act(x)
